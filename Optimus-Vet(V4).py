...
def load_hashes_from_file(file_path):
    with open(file_path, 'r') as file:
        ...
        for line in lines:
            ...
            if (re.match(r'^[a-fA-F0-9]{32}$', lower_hash_value) or  # MD5
                re.match(r'^[a-fA-F0-9]{40}$', lower_hash_value) or  # SHA-1
                re.match(r'^[a-fA-F0-9]{64}$', lower_hash_value) or  # SHA-256
                re.match(r'^[a-fA-F0-9]{128}$', lower_hash_value)):  # SHA-512
...
class HashVettingApp:
    ...
    def refresh_hashes(self):
        ...
        try:
            ...
            # Deduplicate hashes
            self.input_hashes = {h for h in self.input_hashes if re.match(r'^[a-fA-F0-9]{32}$', h) or re.match(r'^[a-fA-F0-9]{40}$', h)}
            self.status_label['text'] = f"Refreshed: {len(self.input_hashes)} valid hashes loaded with {duplicates_count} duplicates removed. Invalid hashes: {', '.join(invalid_hashes)}"
