...
def find_and_compare_hashes(directory, input_hashes, progress_bar, progress_bar_label, root, file_names=None, algorithms=['md5', 'sha1']):
    """Core processing with progress visualization and long path support"""
    file_hashes = {}
    matches = []
    moved_files_info = []
    failed_moves = []
    ...
    # Enable long path support by converting directory to extended path format
    extended_directory = normalize_long_path(directory)
    print(f"Using extended path format: {extended_directory}")
    
    # Ensure quarantine folder exists
    extended_quarantine = normalize_long_path(QUARANTINE_FOLDER)
    Path(extended_quarantine).mkdir(parents=True, exist_ok=True)
    
    # Count files with error handling for long paths
    print("Counting files in target directory...")
    file_count = count_files_safely(extended_directory)
    
    # Set the maximum value for the progress bar
    progress_bar['maximum'] = file_count
    progress_bar_label.config(text=f"Found {file_count} files. Beginning processing...")
    
    # Process files with error handling
    for root_dir, dirs, files in custom_walk(extended_directory):
        for file_name in files:
            try:
                ...
                # File processing continues with the rest of your logic...
                # [Your existing match and move code here]
                
            except Exception as e:
                logging.error(f"Error processing file {os.path.join(root_dir, file_name)}: {e}")
                failed_scans.append((os.path.join(root_dir, file_name), str(e)))
...
def secure_move(file_path):
    ...
    try:
        ...
        extended_file_path = normalize_long_path(file_path)
        extended_dest_folder = normalize_long_path(QUARANTINE_FOLDER)
        
        # Create the destination path
        file_name = os.path.basename(extended_file_path)
        dest_path = os.path.join(extended_dest_folder, file_name)
        ...
        while os.path.exists(dest_path):
            ...
            new_name = f"{base_name}_conflict{attempt}{extension}"
            dest_path = os.path.join(extended_dest_folder, new_name)
            attempt += 1
        ...
        # Move the file
        shutil.move(extended_file_path, dest_path)
        return Path(dest_path), None
        
    except Exception as e:
        error_msg = str(e)
        logging.error(f"Error moving file {file_path}: {error_msg}")
...
class HashVettingApp:
    ...
    def load_data(self):
        ...
        file_names_path = self.file_names_entry.get() or FILE_NAMES_PATH
        if not os.path.isfile(file_names_path):
            self.handle_error(f"File not found: {file_names_path}", "No such file or directory")
            return
