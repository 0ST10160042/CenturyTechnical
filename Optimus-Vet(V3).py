...
def load_hashes_from_file(file_path):
    with open(file_path, 'r') as file:
        ...
        for line in lines:
            ...
            if re.match(r'^[a-fA-F0-9]{32}$', hash_value) or re.match(r'^[a-fA-F0-9]{40}$', hash_value):
        ...
        if len(valid_hashes) != len(lines):
            logging.warning(f"Some lines in {file_path} were not valid hashes and were skipped: {invalid_hashes}")
            self.status_label['text'] = f"Warning: {len(invalid_hashes)} invalid hashes and {duplicates_count} duplicates were skipped."
        else:
            self.status_label['text'] = f"Loaded {len(valid_hashes)} valid hashes with {duplicates_count} duplicates removed."
        return valid_hashes
...
def find_and_compare_hashes(directory, input_hashes, progress_bar, progress_bar_label, root, algorithms=['md5', 'sha1']):
...
class HashVettingApp:
    def __init__(self, root):
        ...
        # Add a refresh function for the load hashes
        self.refresh_hashes_button = tk.Button(self.button_frame, text="Refresh Hashes", command=self.load_hashes, bg='#4e4e4e', fg='white', bd=2, relief='flat', height=1, padx=10, pady=5)
        self.refresh_hashes_button.pack(side=tk.LEFT, expand=True, fill='x', padx=3, pady=2)
        ...
        self.progress_bar_label.grid(row=8, column=0, columnspan=3, sticky='nsew', padx=5, pady=5)
    ...
    def load_hashes(self):
        ...
        try:
            ...
            if len(self.input_hashes) != len(lines):
                invalid_hashes = [line.strip() for line in lines if line.strip() and line.strip().lower() not in self.input_hashes]
                duplicates_count = len(lines) - len(self.input_hashes) - len(invalid_hashes)
                self.status_label['text'] = f"Warning: {len(invalid_hashes)} invalid hashes and {duplicates_count} duplicates were skipped."
                logging.warning(f"Some lines in {hash_file_path} were not valid hashes and were skipped: {invalid_hashes}. {duplicates_count} duplicates were removed.")
            else:
    ...
    def run_scan_and_move(self, target_folder_path):
        self.status_label['text'] = "Scanning..."
        find_and_compare_hashes(Path(target_folder_path), self.input_hashes, self.progress_bar, self.progress_bar_label, self.root)
    ...
    def browse_hash_file(self):
        ...
        if file_path:
            try:
                # Try to open the file in exclusive mode to check if it's open elsewhere
                with open(file_path, 'r+'):  # 'r+' mode allows reading and writing
                    pass
            except IOError:
            ...
            if not os.path.isfile(file_path):
                self.status_label.config(text="Error: File is not saved or does not exist.")
                return
    ...
    def refresh_hashes(self):
        ...
        try:
            ...
            # Deduplicate hashes
            self.input_hashes = {h for h in self.input_hashes if re.match(r'^[a-fA-F0-9]{32}$', h) or re.match(r'^[a-fA-F0-9]{40}$', h)}
            duplicates_count = len(lines) - len(self.input_hashes) - len([line.strip() for line in lines if line.strip() and line.strip().lower() not in self.input_hashes])
